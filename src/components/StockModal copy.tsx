...
import { 
  ...
  ArrowRight, Plus, Trash2, X, Home, Bell, ChevronLeft, ChevronRight, Minus // <--- AÑADIDO CHEVRONRIGHT
} from 'lucide-react';
...
export const StockModal: React.FC<StockModalProps> = ({ isOpen, onClose, householdId }) => {
  ...
  // --- NUEVO: ESTADO DE DETALLE / CONSUMO ---
  const [selectedProduct, setSelectedProduct] = useState<InventoryItem | null>(null); // El producto agrupado seleccionado
  const [consumeAmount, setConsumeAmount] = useState<string>(''); // Cuánto quieres gastar
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: inventoryData } = await supabase
      .from('inventory_items').select('*').eq('household_id', householdId).order('expiry_date', { ascending: true }); // Ordenar por fecha para FIFO fácil
  ...
  // Si estamos viendo un producto y los datos cambian, actualizamos la vista de detalle
  useEffect(() => {
    if (selectedProduct) {
        const updatedGroup = groupedInventory.find(i => i.name === selectedProduct.name);
        if (updatedGroup) setSelectedProduct(updatedGroup);
        else setSelectedProduct(null); // Si desapareció (se gastó todo), volver a lista
    }
  }, [groupedInventory]);
  ...
  // --- LÓGICA DE CONSUMO FIFO (EL CEREBRO DE RESTA) ---
  const handleConsume = async () => {
    if (!selectedProduct || !consumeAmount) return;
    const amountToConsume = Number(consumeAmount);
    if (amountToConsume <= 0) return;

    // 1. Obtener todos los lotes de este producto ordenados por caducidad (FIFO)
    // Usamos rawInventoryItems que ya los tenemos en memoria y filtramos
    const batches = rawInventoryItems
        .filter(i => i.name.toLowerCase() === selectedProduct.name.toLowerCase())
        .sort((a, b) => {
             // Null dates al final, fechas cercanas primero
      ...
      <DialogContent className="bg-zinc-950 border-zinc-800 text-white w-full max-w-2xl max-h-[90vh] h-auto flex flex-col p-0 gap-0">
        ...
        {selectedProduct ? (
             <div className="flex-1 flex flex-col min-h-0 bg-zinc-950">
                 ...
                 <div className="p-6 border-b border-zinc-800 flex flex-col items-center gap-4 bg-zinc-900/30">
                     ...
                     <div className="w-full max-w-xs flex gap-2 items-center bg-zinc-900 p-2 rounded-lg border border-zinc-800">
                         <Input 
                            ...
                            className="bg-transparent border-none text-right text-lg font-bold focus-visible:ring-0"
                            value={consumeAmount}
                     ...
                     <Button 
                        size="lg" 
                        className="w-full max-w-xs bg-red-600 hover:bg-red-500 text-white font-bold shadow-lg shadow-red-900/20"
                        onClick={handleConsume}
                        disabled={!consumeAmount || Number(consumeAmount) <= 0}
                     >
                         <Minus className="w-5 h-5 mr-2" /> Gastar (FIFO)
                     </Button>
                     <p className="text-[10px] text-zinc-500 text-center max-w-xs">
                         El sistema descontará automáticamente de los lotes más antiguos primero.
                     </p>
                 </div>

                 {/* LISTA DE LOTES REALES */}
                 <ScrollArea className="flex-1 p-4">
                     <h4 className="text-xs font-bold text-zinc-500 uppercase mb-3 px-1">Desglose de Lotes</h4>
                     <div className="space-y-2">
                         {rawInventoryItems
                            .filter(i => i.name.toLowerCase() === selectedProduct.name.toLowerCase())
                            .sort((a,b) => new Date(a.expiry_date||'9999').getTime() - new Date(b.expiry_date||'9999').getTime())
                            .map(batch => (
                                <div key={batch.id} className="flex items-center justify-between p-3 rounded-lg border border-zinc-800 bg-zinc-900/50">
                                    ...
                                    <Button variant="ghost" size="icon" onClick={() => handleDeleteInventoryItem(batch.id)} className="text-zinc-600 hover:text-red-500">
                 ...
                 {/* FOOTER: AÑADIR STOCK (NUEVO LOTE) */}
                    ...
                    {filteredGroupedInventory.length === 0 ? <div className="text-center text-zinc-500 py-10 text-sm">Vacío.</div> : filteredGroupedInventory.map(item => {
                            ...
                            return (
                                <div key={item.id} 
                                     ...
                                     className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3 flex items-center gap-3 transition-all hover:bg-zinc-900 cursor-pointer hover:border-blue-500/30 group">
