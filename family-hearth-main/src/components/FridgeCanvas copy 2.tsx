...
export const FridgeCanvas: React.FC<FridgeCanvasProps> = ({ householdId }) => {
  ...
  const runAutomation = useCallback(async () => {
    ...
    // 1. Obtener Datos: Inventario, Recepción y Lista de Compra
    ...
    // Mapa de Recepción (Cosas compradas pero no colocadas)
    ...
    // 3. EVALUACIÓN Y COMPRA AUTOMÁTICA
    for (const [nameKey, stats] of productStats.entries()) {
      // Stock Total Real = (Lo que hay en despensa útil) + (Lo que hay en recepción esperando)
      ...
      // REGLA 3: MANUAL (min_quantity = 0) -> No hace nada aquí

      // ACCIÓN: Añadir a la lista si corresponde y no está ya
      if (priority) {
